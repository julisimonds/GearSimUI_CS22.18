	...
	: GenericPage(parent)
{
	...
	connect(aircraftTypesView->selectionModel(), &QItemSelectionModel::selectionChanged, [=](const QItemSelection& selected) {
		for (const auto& index : selected.indexes()) {
			...
			aircraftDescription->setText(aircraftSelected + " description "); // Must put individual descriptions for each aircraft in if statements
...
void NewProjectPage::nextPage() {
	...
	QString directorySelected = "InsertDirectoryLine82-NewProjectPage.cpp";
